/**
 * BACKTEST: Confluence Radar — Double tulen (tepat 2) vs Triple (3+) vs semua.
...
function summarize(label, rets) {
    if (!rets.length) { console.log(`${label.padEnd(28)} N=  0`); return; }
    const wins = rets.filter(r => r > 0).length;
    const tot = rets.reduce((a, b) => a + b, 0);
    console.log(`${label.padEnd(28)} N=${String(rets.length).padStart(3)} | WR ${(wins / rets.length * 100).toFixed(0).padStart(3)}% | avg ${(tot / rets.length).toFixed(2).padStart(6)}% | TOTAL ${tot.toFixed(1).padStart(7)}% | worst ${Math.min(...rets).toFixed(1)}%`);
}

// Kumpul semua signal confluence (dedup)
const seen = new Set();
const byConf = {};
for (let i = 0; i < dates.length - 3; i++) {
    for (const [name, item] of Object.entries(allData[dates[i]])) {
        if (seen.has(name) || item.price <= 0 || item.price > 10.0 || item.price < 0.10) continue;
        const conf = confluenceCount(item);
        if (conf < 2) continue;
        seen.add(name);
        const fut = futurePrices(name, i, 20);
        if (!fut || !fut.length) continue;
        const r = rideFloor(item.price, item.floorLow || item.price * 0.95, fut);
        (byConf[conf] = byConf[conf] || []).push({ ret: r, name, date: dates[i] });
    }
}

console.log(`Data: ${dates.length} hari (${dates[0]} -> ${dates[dates.length - 1]}), exit=RIDE floor 20h\n`);
for (let c = 2; c <= 5; c++) {
    if (byConf[c]) summarize(`conf=${c} (tepat ${c} strategi)`, byConf[c].map(x => x.ret));
}
const all = Object.values(byConf).flat();
const twoPlus = all.filter(x => byConf[Object.keys(byConf).find(k => k == 2)]);
// kumpulan praktikal
summarize('DOUBLE (2+) semua', all.map(x => x.ret));
summarize('TRIPLE (3+) sahaja', all.filter(x => {
    // perlukan conf sebenar — simpan conf dalam entry
    return true;
}).map(x => x.ret));
// betul: kira semula ikut key
const d2 = [], d3 = [];
for (const [k, v] of Object.entries(byConf)) {
    if (parseInt(k) >= 2) d2.push(...v.map(x => x.ret));
    if (parseInt(k) >= 3) d3.push(...v.map(x => x.ret));
}
console.log('');
summarize('DOUBLE (2+) — semua', d2);
summarize('TRIPLE (3+) sahaja', d3);
