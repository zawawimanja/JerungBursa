...
async function checkMcleanFullHistory() {
    ...
    try {
        ...
        console.log(`🔍 MEMULAKAN SIMULASI SETUP GABUNGAN EMAS BAGI MCLEAN (${prices.length} HARI TRADING)...`);
        ...
        if (triggers.length > 0) {
            console.log(`\n🎉 MCLEAN PERNAH MASUK SETUP KITA PADA TARIKH BERIKUT:`);
            triggers.forEach(t => {
            ...
            console.log(`\n❌ MCLEAN tidak pernah masuk Setup kita dalam 90 hari lepas.`);
        }
        
    } catch (e) {
        console.error(`❌ Gagal memproses MCLEAN: ${e.message}`);
    }
}

checkMcleanFullHistory();
