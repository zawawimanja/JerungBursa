...
async function run() {
    ...
    console.log(`Last 20 days data for SDCG (0321.KL):`);
    let sumTurnover = 0;
    let flatDays = 0;
    ...
    // Comb stock check from scrape-real.js:
    // const isCombStock = (avgTurnover20 < 500000) || (flatPct20 >= 15.0);
    const avgTurnoverLimit = 500000;
    const flatLimit = 15.0;
    console.log(`- isCombStock (avgTurnover < ${avgTurnoverLimit} or flatPct >= ${flatLimit}%): ${(avgTurnover < avgTurnoverLimit) || (flatPct >= flatLimit)}`);
}
