...
function getDiscussion(filePath) {
    ...
    lines.forEach((line, idx) => {
        ...
        try {
            ...
            if (text.toLowerCase().includes('stratus') && text.toLowerCase().includes('mmcs')) {
                out += `\n[File: ${filePath.split('\\').slice(-3).join('\\')} | Line ${idx+1} | Type: ${data.type}]\n`;
                if (data.type === 'USER_INPUT') {
                    out += `USER: ${data.content}\n`;
                } else {
                    out += `MODEL/OTHER: ${data.content ? data.content.substring(0, 800) : ''}\n`;
                }
            }
        } catch (e) {}
    });
...
fs.writeFileSync("c:\\Users\\aaror\\OneDrive - PERTUBUHAN KESELAMATAN SOSIAL\\Desktop\\JerungBursa\\smart-money-tracker\\scratch\\conv_details.txt", result, 'utf8');
console.log('Written conv details');
