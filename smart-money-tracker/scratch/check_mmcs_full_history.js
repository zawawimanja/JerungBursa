...
async function checkMmcsFullHistory() {
    ...
    try {
        ...
        // Find company name/symbol metadata
        ...
        console.log(`🔍 MEMULAKAN SIMULASI SETUP GABUNGAN EMAS BAGI MMCS (${prices.length} HARI TRADING)...`);
        ...
        if (triggers.length > 0) {
            console.log(`\n🎉 MMCS PERNAH MASUK SETUP KITA PADA TARIKH BERIKUT:`);
            triggers.forEach(t => {
            ...
            console.log(`\n❌ MMCS tidak pernah masuk Setup kita dalam 90 hari lepas.`);
        }
        
    } catch (e) {
        console.error(`❌ Gagal memproses MMCS: ${e.message}`);
    }
}

checkMmcsFullHistory();
