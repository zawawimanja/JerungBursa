...
async function checkDnexFullHistory() {
    const ticker = '4456.KL';
    ...
    try {
        ...
        console.log(`\n🔍 MEMULAKAN SIMULASI SETUP GABUNGAN EMAS BAGI DNEX (${prices.length} HARI TRADING)...`);
        ...
        if (triggers.length > 0) {
            console.log(`\n🎉 DNEX PERNAH MASUK SETUP KITA PADA TARIKH BERIKUT:`);
            triggers.forEach(t => {
            ...
            console.log(`\n❌ DNEX tidak pernah masuk Setup kita dalam 90 hari lepas.`);
        }
        
    } catch (e) {
        console.error(`❌ Gagal memproses DNEX: ${e.message}`);
    }
}

checkDnexFullHistory();
