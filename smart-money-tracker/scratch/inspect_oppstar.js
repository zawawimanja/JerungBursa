...
async function inspectOppstar() {
    const symbol = '0275.KL'; // OPPSTAR
    const headers = {
    ...
    try {
        ...
        console.log(`\n📊 Data 15 Hari Terakhir untuk OPPSTAR:`);
        ...
        // Let's find recent support floor
        const last10 = data.slice(-10);
        const floor10 = Math.min(...last10.map(d => d.low));
        let touch10 = 0;
        last10.forEach(d => {
            if (((d.low - floor10) / floor10) * 100 <= 2.0) touch10++;
        });
        console.log(`   - Lantai Sokongan (10-Hari): RM ${floor10.toFixed(3)} (Telah disentuh sebanyak ${touch10} kali)`);
        console.log(`   - Jarak dari Lantai Sokongan: +${(((currentPrice - floor10)/floor10)*100).toFixed(2)}%`);
