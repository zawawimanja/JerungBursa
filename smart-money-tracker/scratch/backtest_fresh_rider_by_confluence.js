/**
 * BACKTEST: Fresh VVIP Rider — adakah ranking ikut CONFLUENCE memberi beza PnL?
 * Split signal Fresh Rider ikut bilangan strategi bertembung (0, 1, 2, 3+).
...
function summarize(label, rets) {
    ...
    console.log(`${label.padEnd(30)} N=${String(rets.length).padStart(3)} | WR ${(wins / rets.length * 100).toFixed(0).padStart(3)}% | avg ${(tot / rets.length).toFixed(2).padStart(6)}% | TOTAL ${tot.toFixed(1).padStart(7)}% | worst ${Math.min(...rets).toFixed(1)}%`);
}

// Kumpul semua signal Fresh Rider (dedup, macam backtest asal)
const seen = new Set();
const groups = { '0 (tiada conf)': [], '1': [], '2 (Double)': [], '3+ (Triple)': [] };
for (let i = 0; i < dates.length - 3; i++) {
    for (const [name, item] of Object.entries(allData[dates[i]])) {
        if (seen.has(name) || item.price < 0.10) continue;
        ...
        const key = conf >= 3 ? '3+ (Triple)' : (conf === 2 ? '2 (Double)' : (conf === 1 ? '1' : '0 (tiada conf)'));
        groups[key].push({ ret: r, name, conf, date: dates[i] });
    }
}

console.log(`Data: ${dates.length} hari (${dates[0]} -> ${dates[dates.length - 1]}), exit=RIDE floor 20h`);
console.log(`Fresh Rider signals (unik): ${seen.size}\n`);

for (const [k, v] of Object.entries(groups)) {
    summarize(`Confluence ${k}`, v.map(x => x.ret));
}

// Juga: korelasi mudah — kumpul ikut conf 0..5 penuh
console.log('\n--- Pecahan penuh ikut bilangan strategi bertembung ---');
const byConf = {};
for (const [k, v] of Object.entries(groups)) for (const x of v) {
    (byConf[x.conf] = byConf[x.conf] || []).push(x.ret);
}
for (let c = 0; c <= 5; c++) {
    if (byConf[c]) summarize(`conf=${c} strategi`, byConf[c]);
}
