...
list.forEach(item => {
    ...
    // Exactly matches the updated filter in index.html line 1290!
    if ((style === 'EXPLOSIVE' || style === 'STAIRCASE') && (item.turnover >= 750000) && !item.isCombStock) {
        picks.push({
            name: item.name,
            style,
            price: item.price,
...
console.log("=== FINAL DASHBOARD RENDERING (TOP 6) ===");
