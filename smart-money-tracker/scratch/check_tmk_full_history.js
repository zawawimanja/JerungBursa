...
async function checkTmkFullHistory() {
    ...
    try {
        ...
        console.log(`🔍 MEMULAKAN SIMULASI SETUP GABUNGAN EMAS BAGI TMK (${prices.length} HARI TRADING)...`);
        ...
        if (triggers.length > 0) {
            console.log(`\n🎉 TMK PERNAH MASUK SETUP KITA PADA TARIKH BERIKUT:`);
            triggers.forEach(t => {
            ...
            console.log(`\n❌ TMK tidak pernah masuk Setup kita dalam 90 hari lepas.`);
        }
        
    } catch (e) {
        console.error(`❌ Gagal memproses TMK: ${e.message}`);
    }
}
