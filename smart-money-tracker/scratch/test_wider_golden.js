...
console.log("=== SIMULATING GOLDEN ZONE WITH DIST <= 3.0% ===");
...
goldenSignals.forEach((item, index) => {
  ...
  console.log(`${index + 1}. Ticker=${item.name.padEnd(8)} | Price=RM ${item.price.toFixed(3)} | Floor=RM ${floor.toFixed(3)} | Touches=${item.touchCount}x | DistToFloor=${dist}% | Setup=${item.setupName}`);
});
